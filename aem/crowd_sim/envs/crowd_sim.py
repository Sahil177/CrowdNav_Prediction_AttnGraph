...
from aem.crowd_sim.envs.utils.human import Human
from aem.crowd_sim.envs.utils.info import *
from aem.crowd_sim.envs.utils.utils import point_to_segment_dist
from aem.crowd_sim.envs.utils.state import FullState
...
class CrowdSim(gym.Env):
    ...
    def find_assignment(self, prev_dets_xy, dets_xy):
        ...
        # match OF preds with current detections with hungarian algo
        # cost matrix
    ...
    def step(self, action, update=True, rebuild=True):
        ...
        if update:
            ...
            print(f'robot_pos: {robot_positions}')
            print(f'robot_pos2: {[self.robot.px, self.robot.py]}')
            dets_xy *= -1
