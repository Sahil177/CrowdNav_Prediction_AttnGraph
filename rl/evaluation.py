...
def evaluate(actor_critic, eval_envs, num_processes, device, test_size, logging, config, args, visualize=False, aem = None):
        ...
        while not done:
            ...
            with torch.no_grad():
                if aem is None:
                    ...
                    obs_aem = obs['aem'][0]
                    print(str(obs_aem.self_state))
                    print([str(human_state) for human_state in obs_aem.human_states])
                    action = aem.predict(obs_aem)
                    action = [action.vx, action.vy]
            if not done:
