...
from aem.crowd_nav.policy.policy_factory import policy_factory
import configparser
...
def main():
	...
	# load aem policy
	policy_config_file = 'aem/crowd_nav/configs/policy.config'
	aem_model_file = 'trained_models_aem/rl_model_5400.pth'
	aem = policy_factory['actenvcarl']()
	policy_config = configparser.RawConfigParser()
	policy_config.read(policy_config_file)
	policy_config.set("actenvcarl", "test_policy_flag", '5')
	policy_config.set("actenvcarl", "multi_process", 'self_attention')
	policy_config.set("actenvcarl", "act_steps", 1)
	policy_config.set("actenvcarl", "act_fixed", False)
	aem.configure(policy_config)
	aem.get_model().load_state_dict(torch.load(aem_model_file))
	aem.set_phase('test')
	aem.set_device(device)
	print(envs)
	aem.time_step = env_config.env.time_step
	...
	evaluate(actor_critic, envs, 1, device, test_size, logging, config, algo_args, test_args.visualize, aem=aem)
